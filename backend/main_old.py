...
# MRI-specific image preprocessing transforms
def get_mri_transforms():
    """Get MRI-specific preprocessing transforms"""
...
# Alternative transforms for different MRI types
def get_enhanced_mri_transforms():
    """Enhanced preprocessing for better MRI analysis"""
    return transforms.Compose([
        ...
        transforms.Lambda(lambda x: (x - x.min()) / (x.max() - x.min() + 1e-8)),  # Min-max normalization
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])
